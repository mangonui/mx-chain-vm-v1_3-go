...
// vmhost/contexts/runtime.go all make+copy the range BEFORE returning,
// so they are safe-by-construction; the deprecation prevents NEW
// callers from skipping that copy and triggering a dangling-after-Grow
// UAF on the wasmer linear-memory alias.
//
// nolint
func (memory *Memory) Data() []byte {
	...
	// self-assignment with `unsafe.Slice`. Aliasing-after-Grow hazard is
